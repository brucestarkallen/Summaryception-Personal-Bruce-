...
function buildContinuityRecord() {
    ...
    if (nb) parts.push('NOTEPAD (manual STARTING canon — deliberately never updated: foundational facts are binding; situational details describe the story\'s OPENING state, and the story having outgrown them is not a contradiction):\n' + nb);
    let led = '';
...
function buildTransplantExport(store, meta) {
    ...
    L.push('');
    L.push('## NOTEPAD (author STARTING canon — highest authority for foundational facts; deliberately never updated: situational details describe the story\'s opening state)');
