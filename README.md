...
- The file‑header comment version is intentionally stale; the real version lives in `manifest.json` **and** the `APP_READY` `console.log` line — keep those two in sync on every release.
